...
private[log] class Cleaner(val id: Int,
  ...
  private[log] def clean(cleanable: LogToClean): (Long, CleanerStats) = {
    ...
    for (group <- groupSegmentsBySize(log.logSegments(0, endOffset), log.config.segmentSize, log.config.maxIndexSize, cleanable.firstUncleanableOffset)) {
      info(s"cleaning log group ${group.map(_.baseOffset)}")
  ...
  private[log] def cleanSegments(log: Log,
    ...
    var cleanedSegment = createCleanedLogSegment(segments.head.baseOffset, log, maxIndexSize, maxTimeIndexSize, indexIntervalBytes)
    ...
    try {
      ...
      for (old <- segments) {
        ...
        info("Cleaning segment %s in log %s (largest timestamp %s) into %s, %s deletes."
            .format(old.baseOffset, log.name, new Date(old.largestTimestamp), cleanedSegment.baseOffset, if(retainDeletes) "retaining" else "discarding"))
        // Hotfix: LIKAFKA-13793. Split segment into multiple segments if needed.
        var done = false
        var startPosition = 0
        while (!done) {
          cleanInto(log.topicPartition, old, cleanedSegment, map, retainDeletes, log.config.maxMessageSize, stats, startPosition) match {
            case Some(CleanIntoResult(nextOffset, nextPosition)) =>
              info(s"Splitting log segment ${old.baseOffset} at offset $nextOffset")
              doneCleaningSegment(cleanedSegment, old.lastModified)
              cleanedSegment = createCleanedLogSegment(nextOffset, log, maxIndexSize, maxTimeIndexSize, indexIntervalBytes)
  ...
  private def createCleanedLogSegment(baseOffset: Long,
    ...
    logFile.delete()
    val indexFile = new File(Log.indexFilename(log.dir, baseOffset).getPath + Log.CleanedFileSuffix)
    val timeIndexFile = new File(Log.timeIndexFilename(log.dir, baseOffset).getPath + Log.CleanedFileSuffix)
    indexFile.delete()
    ...
    val timeIndex = new TimeIndex(timeIndexFile, baseOffset, maxTimeIndexSize)
     new LogSegment(records, index, timeIndex, baseOffset, indexIntervalBytes, log.config.randomSegmentJitter, time)
  }
  ...
  private[log] def cleanInto(topicPartition: TopicPartition,
    ...
    val logCleanerFilter = new LogEntryFilter {
      override def shouldRetain(logEntry: LogEntry): Boolean =
        shouldRetainMessage(source, map, retainDeletes, logEntry, stats)

      override def canBeFilteredTo(logEntry: LogEntry): Boolean = logEntry.offset() <= dest.baseOffset + Int.MaxValue
    }
